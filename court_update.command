...
guard CommandLine.arguments.count >= 2 else {
    printError("Usage: \ncourt_update CASE_NUMBER")
    exit(1)
}

// Extract args
let caseNumber = CommandLine.arguments[1]
...
do {
	...
	printError("You may need to go to Chrome and activate 'View -> Developer -> Allow JavaScript from Apple Events' to enable browser automation")
	exit(1)
}
...
execute(
	js: "document.querySelector('input[id=\\\"txtCaseNumber\\\"]').value = '\(caseNumber)'", 
	tabId: tabId
)
...
_ = getSourceUntilContaining(string: "\(caseNumber)", tabId: tabId)

// Extract dates
guard let mostRecentDate = getMostRecentDateFromSource() else {
	printError("Error: No dates!")
	shell("say No Dates")
	exit(1)
}
